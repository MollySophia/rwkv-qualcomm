...
class qnn_graph(torch.nn.Module):
    def __init__(self, chunk_size, n_head, head_size):
        ...
        if chunk_size == 1:
            ...
            self.wkv_func = torch.ops.rwkv.wkv6_chunk
        else:
            assert False
...
def onnx_custom_wkv_chunk(g, k, v, r, state2, time_first, time_decay):
    out1, out2 = g.op("rwkv::wkv6_chunk", k, v, r, state2, time_first, time_decay, outputs=2)
    return out1.setType(k.type().with_dtype(torch.float32).with_sizes([wkv_chunk_size, n_head, 1, head_size])),\
        out2.setType(k.type().with_dtype(torch.float32).with_sizes([n_head, head_size, head_size]))
from torch.onnx import register_custom_op_symbolic
register_custom_op_symbolic("rwkv::wkv6", onnx_custom_wkv, 9)
register_custom_op_symbolic("rwkv::wkv6_chunk", onnx_custom_wkv_chunk, 9)
...
torch.onnx.export(gen_graph, inputs, "test_wkv.onnx", output_names=["output", "state"], opset_version=9)
subprocess.call("qnn-onnx-converter -i test_wkv.onnx --op_package_config ../RwkvWkvOpPackageHTP.xml", stdout=cmd_stdout, stderr=cmd_stderr, shell=True)
...
print(qnn_output.flatten()[:10])
print(torch_output.flatten()[:10])
...
print(qnn_state.flatten()[:10])
print(torch_state.flatten()[:10])
if not torch.allclose(qnn_state, torch_state, rtol=1e-5, atol=1e-5):
    print("!!!wkv length=1 state mismatch!!!")
else:
    print("wkv length=1 state passed")
...
subprocess.call("qnn-onnx-converter -i test_wkv_chunk.onnx --op_package_config ../RwkvWkvOpPackageHTP.xml", stdout=cmd_stdout, stderr=cmd_stderr, shell=True)
...
print("executing qnn-net-run")
subprocess.call(f"qnn-net-run --input_list input_list_wkv_chunk.txt --model lib/x86_64-linux-clang/libtest_wkv_chunk.so --backend {qnn_sdk_root}/lib/x86_64-linux-clang/libQnnHtp.so --op_packages ../HTP/RwkvWkvOpPackage/build/x86_64-linux-clang/libQnnRwkvWkvOpPackage.so:RwkvWkvOpPackageInterfaceProvider", stdout=cmd_stdout, stderr=cmd_stderr, shell=True)
...
print(torch_state.flatten()[:10])
if not torch.allclose(qnn_state, torch_state, rtol=1e-5, atol=1e-5):
    print(f"!!!wkv length={wkv_chunk_size} state mismatch!!!")
else:
    print(f"wkv length={wkv_chunk_size} state passed")
