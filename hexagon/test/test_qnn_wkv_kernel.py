...
def onnx_custom_wkv(g, k, v, r, state2, time_first, time_decay):
    seq_length = k.type().sizes()[0]
    out1, out2 = g.op("rwkv::wkv6", k, v, r, state2, time_first, time_decay, outputs=2)
    return out1.setType(k.type().with_dtype(torch.float32).with_sizes([seq_length, n_head, 1, head_size])),\
...
def test_wkv6_cpu(n_head, head_size, seq_length):
    ...
    td = torch.rand(seq_length, n_head, head_size, 1)
    ...
    qnn_output = torch.from_numpy(np.fromfile("output/Result_0/output.raw", dtype=np.float32)).reshape(seq_length, n_head, 1, head_size)
