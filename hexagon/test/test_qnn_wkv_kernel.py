...
def test_wkv7_htp(n_head, head_size, seq_length):
    ...
    print("!!! converting test graph")
    torch.onnx.export(gen_graph, inputs, "test_wkv.onnx", output_names=["output", "state"], opset_version=17)
    subprocess.call("qnn-onnx-converter -i test_wkv.onnx --input_layout state.1 NFC --op_package_config ../RwkvWkvOpPackageHTP.xml", stdout=cmd_stdout, stderr=cmd_stderr, shell=True)
