...
DECLARE_PKG_OPS_OPTS_LIST(PKG_wkv7_output_x)
DECLARE_PKG_OPS_OPTS_LIST(PKG_wkv7_output_state)
...
static std::array<const char*, 4> sg_opNames{{"wkv7", "wkv7_output_x", "wkv7_output_state", "wkv6"}};
 ...
 * can be set by Qnn_ErrorHandle_t RwkvWkvOpPackageLogSetLevel(QnnLog_Level_t maxLogLevel)
 */
/*
...
Qnn_ErrorHandle_t RwkvWkvOpPackageInit(QnnOpPackage_GlobalInfrastructure_t infrastructure) {
...
Qnn_ErrorHandle_t RwkvWkvOpPackageGetInfo(const QnnOpPackage_Info_t** info) {
...
Qnn_ErrorHandle_t RwkvWkvOpPackageLogInitialize(QnnLog_Callback_t callback, QnnLog_Level_t maxLogLevel) {
...
Qnn_ErrorHandle_t RwkvWkvOpPackageLogSetLevel(QnnLog_Level_t maxLogLevel) {
    if (maxLogLevel < QNN_LOG_LEVEL_ERROR) return QNN_LOG_ERROR_INVALID_ARGUMENT;
    sg_maxLogLevel = maxLogLevel;
    return QNN_SUCCESS;
}

Qnn_ErrorHandle_t RwkvWkvOpPackageLogTerminate() {
...
Qnn_ErrorHandle_t RwkvWkvOpPackageValidateOpConfig (Qnn_OpConfig_t opConfig){
    ...
    if (std::string(opConfig.v1.typeName) == "wkv7"){
        if (opConfig.v1.numOfParams != 0 || opConfig.v1.numOfInputs != 7 || opConfig.v1.numOfOutputs != 1){
          return QNN_OP_PACKAGE_ERROR_VALIDATION_FAILURE;
        }
    }
    else if (std::string(opConfig.v1.typeName) == "wkv7_output_x"){
        if (opConfig.v1.numOfParams != 0 || opConfig.v1.numOfInputs != 1 || opConfig.v1.numOfOutputs != 1){
          return QNN_OP_PACKAGE_ERROR_VALIDATION_FAILURE;
        }
    }
    else if (std::string(opConfig.v1.typeName) == "wkv7_output_state"){
        if (opConfig.v1.numOfParams != 0 || opConfig.v1.numOfInputs != 1 || opConfig.v1.numOfOutputs != 1){
          return QNN_OP_PACKAGE_ERROR_VALIDATION_FAILURE;
        }
    }
    else if (std::string(opConfig.v1.typeName) == "wkv6"){
 ...
 * (version 1.3.0) Qnn_ErrorHandle_t RwkvWkvOpPackageCreateKernels (QnnOpPackage_GraphInfrastructure_t
 * graphInfrastructure, QnnOpPackage_Node_t node, QnnOpPackage_Kernel_t** kernels, uint32_t*
 * numKernels)
 * (version 1.3.0) Qnn_ErrorHandle_t RwkvWkvOpPackageFreeKernels (QnnOpPackage_Kernel_t* kernels)
 *
 * (version 1.4.0) Qnn_ErrorHandle_t RwkvWkvOpPackageCreateOpImpl (QnnOpPackage_GraphInfrastructure_t
 * graphInfrastructure, QnnOpPackage_Node_t node, QnnOpPackage_OpImpl_t* opImpl)
 *(version 1.4.0) Qnn_ErrorHandle_t RwkvWkvOpPackageFreeOpImpl (QnnOpPackage_OpImpl_t opImpl)
 */

Qnn_ErrorHandle_t RwkvWkvOpPackageTerminate() {
if (!sg_packageInitialized) return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
/* latest version */
Qnn_ErrorHandle_t RwkvWkvOpPackageInterfaceProvider(QnnOpPackage_Interface_t* interface) {
  if (!interface) return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
  interface->interfaceVersion      = {1, 4, 0};
  interface->v1_4.init             = RwkvWkvOpPackageInit;
