...
if type(model) == list:
    ...
    for i in range(len(model)):
        ...
        converter_cmd = f"{qnn_sdk_root}/bin/{qnn_tools_target}/qnn-onnx-converter -i {onnx_path} --float_bitwidth {parser_args.qnn_float_width} --act_bitwidth 16 --bias_bitwidth 8 "
    ...
    states_layout = "NONTRIVIAL"
    converter_cmd = f"{qnn_sdk_root}/bin/{qnn_tools_target}/qnn-onnx-converter -i {onnx_output_path} --float_bitwidth {parser_args.qnn_float_width} --act_bitwidth 16 --bias_bitwidth 8 " + " ".join([f'--input_layout "state{3*j+1}_in" "{states_layout}"' for j in range(model.layer_begin, model.layer_end)])
